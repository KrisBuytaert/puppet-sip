...
# Then transfer files to remote nodes 
	...
	file {
		...
		"/usr/bin/ssl-cert/ca_rehash":
  			source => "puppet:///sipx/ssl-cert/ca_rehash",
  			mode => 755,
               		ensure => present;

    		"/usr/bin/ssl-cert/check-cert.sh":
                        source => "puppet:///sipx/ssl-cert/check-cert.sh",
                        mode => 755,
                        ensure => present;

 		 "/usr/bin/ssl-cert/gen-ssl-keys.sh":
                        source => "puppet:///sipx/ssl-cert/gen-ssl-keys.sh",
                        mode => 755,
                        ensure => present;

    		"/usr/bin/ssl-cert/install-cert.sh":
                        source => "puppet:///sipx/ssl-cert/install-cert.sh",
                        mode => 755,
                        ensure => present;

    		"/usr/bin/ssl-cert/upgrade-cert.sh":
                        source => "puppet:///sipx/ssl-cert/upgrade-cert.sh",
                        mode => 755,
                        ensure => present;


   		"/usr/local/bin/genssl.sh":
  			source => "puppet:///sipx/genssl.sh",
  			mode => 755,
               		ensure => present;
...
#			ensure => directory;
			...
			source => "puppet:///sipx/certdb/ca.$platformdomainextension.crt",
                        owner => "sipxchange", group => "sipxchange",
                        ensure => present;
      		"/var/sipxdata/certdb/ca.$platformdomainextension.csr":
			source => "puppet:///sipx/certdb/ca.$platformdomainextension.csr",
                        owner => "sipxchange", group => "sipxchange",
                        ensure => present;
      		"/var/sipxdata/certdb/ca.$platformdomainextension.key":
			source => "puppet:///sipx/certdb/ca.$platformdomainextension.key",
                        owner => "sipxchange", group => "sipxchange",
                        ensure => present;
      		"/var/sipxdata/certdb/ca.$platformdomainextension.ser":
			source => "puppet:///sipx/certdb/ca.$platformdomainextension.ser",
                        owner => "sipxchange", group => "sipxchange",
                        ensure => present;
      		"/var/sipxdata/certdb/rnd_seed":
			source => "puppet:///sipx/certdb/rnd_seed",
                        owner => "sipxchange", group => "sipxchange",
                        ensure => present;
	}
...
define sipx::staticcertdbnodes($clientname){
   ...
   file {
      		...
      		"/var/sipxdata/certdb/$clientname.$platformdomainextension.crt":
                        source => "puppet:///sipx/certdb/$clientname.$platformdomainextension.crt",
                        owner => "sipxchange", group => "sipxchange",
                        mode => 600,
                        ensure => present;
      		"/var/sipxdata/certdb/$clientname.$platformdomainextension.csr":
                        source => "puppet:///sipx/certdb/$clientname.$platformdomainextension.csr",
                        owner => "sipxchange", group => "sipxchange",
                        mode => 600,
                        ensure => present;
      		"/var/sipxdata/certdb/$clientname.$platformdomainextension.key":
                        source => "puppet:///sipx/certdb/$clientname.$platformdomainextension.key",
                        owner => "sipxchange", group => "sipxchange",
                        mode => 600,
                        ensure => present;

		"/var/sipxdata/certdb/$clientname.$platformdomainextension.p12":
                        source => "puppet:///sipx/certdb/$clientname.$platformdomainextension.p12",
                        owner => "sipxchange", group => "sipxchange",
                        mode => 600,
                        ensure => present;

  		"/var/sipxdata/certdb/$clientname.${platformdomainextension}_crt.cfg":
                        source => "puppet:///sipx/certdb/$clientname.${platformdomainextension}_crt.cfg",
                        owner => "sipxchange", group => "sipxchange",
                        mode => 600,
                        ensure => present;
	}
...
# Docs from http://wiki.sipfoundry.org/pages/viewpage.action?pageId=9928981
	...
	file {
		"/etc/sipxpbx/ssl/ssl.crt": 
			source => "puppet:///sipx/certdb/$hostname.$platformdomainextension.crt",
			owner => "sipxchange", group => "sipxchange",
  			mode => 600,
			ensure => present;
		"/etc/sipxpbx/ssl/ssl.key": 
			source => "puppet:///sipx/certdb/$hostname.$platformdomainextension.key",
			owner => "sipxchange", group => "sipxchange",
  			mode => 600,
			ensure => present;
