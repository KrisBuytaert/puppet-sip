...
InstallPrefix=""
##InstallDir="/etc/sipxpbx"
InstallDir="/etc/puppet/modules/sipx/files/certdb"
...
if  /usr/bin/ssl-cert/check-cert.sh ${AuthoritySwitch} "${AuthorityTarget}" "$WorkDir/$Basename.crt"
then
	...
	installedCaName=$( echo "${caName}" | sed "s/ //g" )
        ...
        /usr/bin/ssl-cert/ca_rehash "${InstallPrefix}${InstallDir}/ssl/authorities"
	...
	then
        ...
        then
            ...
            /usr/bin/ssl-cert/check-cert.sh "${InstallDir}/ssl/ssl${installQualifier}.crt"
            certOk+=$?
        fi
    fi
